...
class SequenceParams:
    ...
    frame_rate_hz: float = 1.0      # Frames per second
...
def simulate_example(save_gif: bool = False, cp = ClutterParams()) -> None:
    ...
    # Random initial range and doppler_hz
    init_rng = random.randint(0, rp.n_ranges - 1)
    init_doppler_hz = random.uniform(-100.0, 100.0)  # Doppler in Hz, adjust as needed
    tgt = Target(rng_idx=init_rng, doppler_hz=init_doppler_hz, power=0.02)
    rdm_list = simulate_sequence_with_realistic_target(rp, cp, sp, tgt)
...
def simulate_sequence_with_realistic_target(
    rp: RadarParams,
    cp: ClutterParams,
    sp: SequenceParams,
    tgt: Target,
    ...
    for frame_idx in range(sp.n_frames):
        ...
        # Add target at current position
        add_target_blob(clutter_td, tgt, rp)
        # Calculate velocity from doppler_hz
        velocity = tgt.doppler_hz * rp.carrier_wavelength / 2.0  # m/s
        # Update target range index based on velocity
        rng_float = tgt.rng_idx * rp.range_resolution + velocity * dt
        tgt.rng_idx = int(np.clip(rng_float / rp.range_resolution, 0, rp.n_ranges - 1))
        rdm_list.append(compute_range_doppler(clutter_td, rp, cp))
    return rdm_list
