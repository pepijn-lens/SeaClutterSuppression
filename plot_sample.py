...
def plot_sample(index=5000):
    # Load the data
    data = torch.load('local_data/20SNR_clutter.pt')
    ...
    range_resolution = metadata['range_resolution']  # in meters
    ...
    im1 = ax1.imshow(rd_map[0].squeeze(), 
                    ...
                    extent=[velocity_axis[0], velocity_axis[-1], 
                           range_axis[-1], range_axis[0]])  # Note: range is flipped for proper display
