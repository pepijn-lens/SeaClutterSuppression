...
def comprehensive_evaluation(dataset_path, model_path, base_filter_size, save=None, clustering_params=None, marimo_var=False, distance_threshold=5.0, threshold=0.00001):
...
def interactive_sample_explorer(dataset_path, model_path, base_filter_size, clustering_params=None, distance_threshold=5.0, threshold=0.00001):
    """
    ...
    print(f"Using U-Net confidence threshold: {threshold}")
    print("\n" + "="*60)
    ...
    print("  'unet_threshold X' to change U-Net confidence threshold (e.g., 'unet_threshold 0.0001')")
    ...
    while True:
        try:
            ...
            if user_input in ['quit', 'q', 'exit']:
            ...
            elif user_input.startswith('unet_threshold'):
                try:
                    new_unet_threshold = float(user_input.split()[1])
                    # Update model threshold
                    model.threshold = new_unet_threshold
                    print(f"U-Net confidence threshold updated to {new_unet_threshold}")
                    continue
                except (IndexError, ValueError):
                    print("Invalid U-Net threshold format. Use 'unet_threshold X' where X is a number (e.g., 'unet_threshold 0.0001')")
                    continue
                ...
                try:
                    ...
                    print("Invalid input. Please enter a number, 'random', 'threshold X', 'unet_threshold X', or 'quit'")
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-path", type=str, default="evaluation_results",
    ...
    parser.add_argument("--threshold", type=float, default=0.00001,
                        help="Threshold for U-Net confidence map (default: 0.00001)")
